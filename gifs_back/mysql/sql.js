 ...
 *************************************************************/
//(1) 查询表中所有数据的总数
var DATAS_COUNT = (tableName) => `SELECT COUNT(*) FROM ${tableName}`;
//(2) 查询表中所有数据的总数
var QUERY_DATA_BY_ID = (tableName, id) => `SELECT * FROM ${tableName} WHERE id=${id}`;
//(3) 查询表中id在某个区间到所有数据
var QUERY_DATA_BY_ID_IN_RANGE = (tableName, from, to) => `SELECT * FROM ${tableName} WHERE id>${from} and id<=${to} ORDER BY id DESC`;
/************************约束/外键/字符集相关************************
 * 约束/外键/字符集相关
 *************************************************************/
//(1)
